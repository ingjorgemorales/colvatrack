...
<template>
  ...
  <AppLayout title="Usuarios">
    <section class="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div class="hidden sm:block"><table class="w-full text-left text-sm"><thead class="bg-slate-50 text-slate-500"><tr><th class="px-3 py-2">Usuario</th><th>Cedula</th><th>Correo</th><th>Telefono</th><th>Cargo</th><th>Rol</th><th>Vehiculo</th><th>Estado</th><th class="text-right">Acciones</th></tr></thead><tbody><tr v-for="user in users.data" :key="user.id" class="border-t border-slate-100"><td class="px-3 py-3 font-medium text-slate-900">{{ user.name }} {{ user.last_name }}</td><td>{{ user.cedula ?? '-' }}</td><td>{{ user.email }}</td><td>{{ user.phone ?? '-' }}</td><td>{{ user.cargo ?? '-' }}</td><td>{{ user.role?.name ?? '-' }}</td><td>{{ user.assigned_vehicle?.plate ?? '-' }}</td><td><span class="rounded px-2 py-1" :class="user.status === 'active' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'">{{ user.status }}</span></td><td class="text-right"><Link :href="`/usuarios/${user.id}/edit`" class="mr-2 inline-flex rounded-md border border-slate-200 p-2 text-[#123f6e]"><Pencil class="h-4 w-4" /></Link><button @click="deactivate(user)" class="inline-flex cursor-pointer rounded-md border border-red-200 p-2 text-red-700 transition-colors hover:bg-red-50"><Trash2 class="h-4 w-4" /></button></td></tr></tbody></table></div>
