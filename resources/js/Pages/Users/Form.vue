...
const form = useForm({
  role_id: props.user?.role_id ?? '', name: props.user?.name ?? '', last_name: props.user?.last_name ?? '', cedula: props.user?.cedula ?? '', email: props.user?.email ?? '', phone: props.user?.phone ?? '', cargo: props.user?.cargo ?? '', status: props.user?.status ?? 'active',
  must_change_password: props.user?.must_change_password ?? true, vehicle_id: props.user?.assigned_vehicle?.id ?? '',
});
  ...
  <AppLayout :title="isEdit ? 'Editar usuario' : 'Nuevo usuario'">
    <section class="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <form class="grid gap-4 md:grid-cols-2" @submit.prevent="submit">
        ...
        <label><span class="text-sm font-medium text-slate-600">Telefono <span class="text-red-600">*</span></span><input v-model="form.phone" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-3" required /></label>
        <label><span class="text-sm font-medium text-slate-600">Cargo <span class="text-red-600">*</span></span><input v-model="form.cargo" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-3" required /></label>
        <label><span class="text-sm font-medium text-slate-600">Rol <span class="text-red-600">*</span></span><select v-model="form.role_id" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-3" required><option value="">Seleccionar</option><option v-for="role in roles" :key="role.id" :value="role.id">{{ role.name }}</option></select></label>
        <label><span class="text-sm font-medium text-slate-600">Vehiculo si es conductor</span><select v-model="form.vehicle_id" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-3"><option value="">Sin asignar</option><option v-for="vehicle in vehicles" :key="vehicle.id" :value="vehicle.id">{{ vehicle.plate }} - {{ vehicle.brand }}</option></select><small class="mt-1 block text-xs text-slate-500">Solo aparecen vehiculos activos sin conductor asignado. Si el usuario queda inactivo, se libera su vehiculo.</small></label>
        <label><span class="text-sm font-medium text-slate-600">Estado <span class="text-red-600">*</span></span><select v-model="form.status" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-3" required><option value="active">Activo</option><option value="inactive">Inactivo</option></select></label>
