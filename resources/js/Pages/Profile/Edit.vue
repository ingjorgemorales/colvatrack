  ...
  <AppLayout title="Perfil">
    <section class="grid gap-6 xl:grid-cols-2">
      ...
      <form class="rounded-md border border-slate-200 bg-white p-5 shadow-sm" @submit.prevent="passwordForm.patch('/perfil/password')">
        ...
        <div class="grid gap-4"><input v-model="passwordForm.current_password" type="password" placeholder="Contrasena actual" class="rounded-md border border-slate-300 px-3 py-3" /><input v-model="passwordForm.password" type="password" placeholder="Nueva contraseña" class="rounded-md border border-slate-300 px-3 py-3" /><input v-model="passwordForm.password_confirmation" type="password" placeholder="Confirmar nueva contraseña" class="rounded-md border border-slate-300 px-3 py-3" /></div>
        ...
        <button class="mt-5 inline-flex items-center gap-2 rounded-md bg-[#123f6e] px-5 py-3 font-semibold text-white"><Save class="h-5 w-5" /> Actualizar contraseña</button>
      </form>
    </section>
  </AppLayout>
</template>
