  ...
  <AppLayout title="Perfil">
    <section class="grid gap-6 xl:grid-cols-2">
      ...
      <form class="rounded-md border border-slate-200 bg-white p-5 shadow-sm" @submit.prevent="passwordForm.patch('/perfil/password')">
        ...
        <button class="mt-5 inline-flex cursor-pointer items-center gap-2 rounded-md bg-[#123f6e] px-5 py-3 font-semibold text-white transition-colors hover:bg-[#0e2d52]" :disabled="passwordForm.processing || !passwordsMatch()"><Save class="h-5 w-5" /> Actualizar contraseña</button>
      </form>
    </section>
  </AppLayout>
</template>
