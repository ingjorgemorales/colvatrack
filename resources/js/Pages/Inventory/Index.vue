    ...
    <section class="grid gap-6 xl:grid-cols-[1fr_360px]">
      <div class="space-y-5">
        <article v-for="vehicle in vehicles" :key="vehicle.id" class="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <form v-if="canManageCatalog && items.length" class="mt-3 flex flex-wrap items-center gap-2" @submit.prevent="assignToVehicle(vehicle)"><select v-model="addForm.inventory_item_id" class="rounded border border-slate-300 px-3 py-2 text-sm"><option value="">Agregar herramienta...</option><option v-for="item in items.filter(i => !assignedItemIds(vehicle).has(i.id))" :key="item.id" :value="item.id">{{ item.name }} ({{ item.category?.name }})</option></select><input v-model="addForm.quantity_total" type="number" placeholder="Total" class="w-20 rounded border border-slate-300 px-2 py-2 text-sm" /><input v-model="addForm.quantity_available" type="number" placeholder="Disp." class="w-20 rounded border border-slate-300 px-2 py-2 text-sm" /><button class="rounded bg-green-600 px-3 py-2 text-sm font-semibold text-white" :disabled="addForm.processing || !addForm.inventory_item_id"><Plus class="h-4 w-4 inline" /> Asignar</button></form>
        </article>
      </div>
