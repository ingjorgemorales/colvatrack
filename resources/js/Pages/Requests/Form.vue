...
function addItem(){ if (selectedIsOccupied.value) return; const first = availableItems.value[0]; if(first) form.items.push({ inventory_item_id: first.inventory_item_id, quantity: 1 }); }
    ...
    <section class="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <section class="mb-5 rounded-md border border-emerald-200 bg-emerald-50 p-4">
        ...
        <div v-else-if="nearbyVehicles.length" class="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <button v-for="v in nearbyVehicles" :key="v.id" type="button" :disabled="v.is_occupied" @click="selectVehicle(v.id)" class="rounded-md border bg-white p-3 text-left shadow-sm transition" :class="[Number(form.vehicle_id) === Number(v.id) ? 'border-emerald-600 ring-2 ring-emerald-200' : 'border-slate-200', v.is_occupied ? 'cursor-not-allowed opacity-70' : 'hover:border-emerald-500']">
        ...
        <button v-if="nearestVehicle" type="button" @click="selectVehicle(nearestVehicle.id)" class="mt-4 cursor-pointer rounded-md bg-emerald-700 px-4 py-2 text-sm font-bold text-white transition-colors hover:bg-emerald-800">Usar mas cercano: {{ nearestVehicle.plate }} ({{ formatDistance(nearestVehicle.distance_meters) }})</button>
      </section>
      ...
      <form class="space-y-5" @submit.prevent="form.post('/solicitudes')">
        ...
        <button :disabled="selectedIsOccupied || form.processing" class="rounded-md bg-[#123f6e] px-5 py-3 font-semibold text-white transition-colors" :class="selectedIsOccupied || form.processing ? 'cursor-not-allowed opacity-60' : 'cursor-pointer hover:bg-[#0e2d52]'">Crear solicitud</button>
      </form>
    </section>
  </AppLayout>
</template>
