...
<template>
  ...
  <AppLayout title="Vehiculos">
    <section class="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
      <div class="mb-5 grid gap-3 xl:grid-cols-[1fr_220px_150px_auto_auto]">
      ...
      <div v-if="vehicles.last_page > 1" class="mt-5 flex flex-wrap items-center justify-center gap-2">
        <Link v-for="link in vehicles.links" :key="link.label" :href="link.url || '#'" preserve-scroll class="rounded-md border px-3 py-2 text-sm font-semibold" :class="[link.active ? 'border-[#123f6e] bg-[#123f6e] text-white' : 'border-slate-200 bg-white text-slate-700', !link.url ? 'pointer-events-none opacity-40' : 'hover:bg-[#edf3fa]']" v-html="link.label" />
      </div>
    </section>
  </AppLayout>
</template>
