...
const apply = () => router.get('/vehiculos', { search: search.value, status: status.value, movement: movement.value, date_from: dateFrom.value, date_to: dateTo.value, per_page: perPage.value }, { preserveState: true, replace: true });
const clearFilters = () => { search.value = ''; status.value = ''; movement.value = ''; dateFrom.value = ''; dateTo.value = ''; perPage.value = 10; router.get('/vehiculos', {}, { preserveState: true, replace: true }); };
...
<template>
  ...
  <AppLayout title="Vehiculos">
    <section class="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
      <div class="mb-5 grid gap-2 md:grid-cols-2 xl:grid-cols-[1fr_160px_220px_150px_150px_140px_auto_auto]">
        ...
        <select v-model="movement" class="w-full rounded-md border border-slate-300 px-3 py-3 sm:w-auto"><option value="">Todos</option><option value="moving_current">En movimiento ahora</option><option value="stopped_current">Sin movimiento ahora</option><option value="with_gps_in_range">Con GPS en el rango</option><option value="moved_in_range">Se movio en el rango</option><option value="not_moved_in_range">No se movio en el rango</option><option value="no_gps_in_range">Sin GPS en el rango</option></select>
        <input v-model="dateFrom" type="date" class="w-full rounded-md border border-slate-300 px-3 py-3 sm:w-auto" />
        <input v-model="dateTo" type="date" class="w-full rounded-md border border-slate-300 px-3 py-3 sm:w-auto" />
