...
const canEditVehicles = computed(() => can('vehiculos', 'editar'));
const canChangeVehicleStatus = computed(() => can('vehiculos', 'estado'));
const canViewVehicleRoute = computed(() => can('vehiculos', 'recorrido'));
const canViewProjects = computed(() => can('proyectos', 'ver'));
const canReserveVehicles = computed(() => can('reservas_vehiculos', 'crear'));
const canReleaseVehicleReservations = computed(() => can('reservas_vehiculos', 'editar'));
const movementBadgeClass = (status) => ({
  ...
  <AppLayout title="Vehiculos">
    <section class="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
      <div class="mb-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-[1fr_160px_170px_170px_140px_auto_auto]">
        ...
        <select v-model="movement" class="w-full rounded-md border border-slate-300 px-3 py-3 sm:w-auto"><option value="">Todos los movimientos</option><option value="moving">En movimiento</option><option value="stopped">Sin movimiento</option><option value="stale">GPS sin actualizar</option></select>
      ...
      <div class="mb-4 flex flex-wrap items-center justify-between gap-3 text-sm text-slate-600">
        ...
        <div class="flex flex-wrap items-center gap-2">
          ...
          <Link v-if="canViewProjects" href="/vehiculos/proyectos" class="inline-flex items-center gap-2 rounded-md border border-slate-200 bg-white px-3 py-2 font-semibold text-[#123f6e] shadow-sm transition-colors hover:bg-[#edf3fa]"><FolderKanban class="h-4 w-4" /> Proyectos</Link>
        </div>
      </div>
        ...
        <table class="w-full text-left text-sm">
          ...
          <tbody>
            <tr v-for="v in vehicles.data" :key="v.id" class="border-t border-slate-100">
              ...
              <td><div>{{ v.current_speed ?? 0 }} km/h</div><span class="mt-1 inline-flex rounded px-2 py-1 text-xs font-semibold" :class="movementBadgeClass(v.movement_status)">{{ v.movement_status_label }}</span></td>
              ...
              <td class="text-right"><Link v-if="canViewVehicleRoute" :href="`/vehiculos/${v.id}/recorrido`" class="mr-2 inline-flex items-center gap-2 rounded-md border border-slate-200 px-3 py-2 font-semibold text-[#123f6e]"><MapPinned class="h-4 w-4" /> Recorrido</Link><button v-if="canReserveVehicles && v.status === 'active' && !v.active_reservation" @click="reserve(v)" class="mr-2 inline-flex cursor-pointer rounded-md border border-amber-200 p-2 text-amber-700 transition-colors hover:bg-amber-50" title="Reservar vehiculo"><LockKeyhole class="h-4 w-4" /></button><button v-if="canReleaseVehicleReservations && v.active_reservation" @click="releaseReservation(v)" class="mr-2 inline-flex cursor-pointer rounded-md border border-emerald-200 p-2 text-emerald-700 transition-colors hover:bg-emerald-50" title="Liberar reserva"><UnlockKeyhole class="h-4 w-4" /></button><Link v-if="canEditVehicles" :href="`/vehiculos/${v.id}/edit`" class="mr-2 inline-flex rounded-md border border-slate-200 p-2 text-[#123f6e]"><Pencil class="h-4 w-4" /></Link><button v-if="canChangeVehicleStatus" @click="toggleStatus(v)" class="inline-flex cursor-pointer rounded-md border p-2 transition-colors" :class="v.status === 'active' ? 'border-orange-200 text-orange-700 hover:bg-orange-50' : 'border-emerald-200 text-emerald-700 hover:bg-emerald-50'" :title="v.status === 'active' ? 'Desactivar vehiculo' : 'Activar vehiculo'"><PowerOff v-if="v.status === 'active'" class="h-4 w-4" /><Power v-else class="h-4 w-4" /></button></td>
            </tr>
      ...
      <div class="space-y-2 sm:hidden"><div v-for="v in vehicles.data" :key="v.id" class="rounded border border-slate-100 bg-slate-50 p-3 text-sm"><div class="mb-2 flex items-start justify-between gap-2"><div class="font-semibold text-slate-950">{{ v.plate }}</div><div class="flex shrink-0 gap-1"><span v-if="v.active_reservation" class="rounded bg-amber-100 px-2 py-1 text-xs font-semibold text-amber-800">Reservado</span><span class="rounded px-2 py-1 text-xs" :class="v.status === 'active' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'">{{ v.status }}</span></div></div><div class="text-xs text-slate-600">{{ v.brand ?? '-' }} {{ v.model ?? '' }}</div><div class="mt-1 grid grid-cols-2 gap-x-3 gap-y-1 text-xs text-slate-600"><div><span class="font-medium text-slate-700">Proyecto:</span> {{ v.project?.name ?? 'Sin proyecto' }}</div><div><span class="font-medium text-slate-700">Conductor:</span> {{ v.driver?.name ?? 'Sin asignar' }}</div><div><span class="font-medium text-slate-700">GPS:</span> {{ v.provider?.name ?? '-' }}</div><div><span class="font-medium text-slate-700">Velocidad:</span> {{ v.current_speed ?? 0 }} km/h</div><div class="col-span-2"><span class="font-medium text-slate-700">Movimiento:</span> <span class="rounded px-2 py-1 text-xs font-semibold" :class="movementBadgeClass(v.movement_status)">{{ v.movement_status_label }}</span></div><div class="col-span-2"><span class="font-medium text-slate-700">Ultima GPS:</span> {{ v.last_gps_datetime ?? '-' }}</div></div><div class="mt-2 flex flex-wrap gap-2"><Link v-if="canViewVehicleRoute" :href="`/vehiculos/${v.id}/recorrido`" class="inline-flex items-center gap-1 rounded-md border border-slate-200 px-3 py-2 text-xs font-semibold text-[#123f6e]"><MapPinned class="h-3 w-3" /> Recorrido</Link><button v-if="canReserveVehicles && v.status === 'active' && !v.active_reservation" @click="reserve(v)" class="inline-flex cursor-pointer rounded-md border border-amber-200 p-2 text-amber-700 transition-colors hover:bg-amber-50"><LockKeyhole class="h-4 w-4" /></button><button v-if="canReleaseVehicleReservations && v.active_reservation" @click="releaseReservation(v)" class="inline-flex cursor-pointer rounded-md border border-emerald-200 p-2 text-emerald-700 transition-colors hover:bg-emerald-50"><UnlockKeyhole class="h-4 w-4" /></button><Link v-if="canEditVehicles" :href="`/vehiculos/${v.id}/edit`" class="inline-flex rounded-md border border-slate-200 p-2 text-[#123f6e]"><Pencil class="h-4 w-4" /></Link><button v-if="canChangeVehicleStatus" @click="toggleStatus(v)" class="inline-flex cursor-pointer rounded-md border p-2 transition-colors" :class="v.status === 'active' ? 'border-orange-200 text-orange-700 hover:bg-orange-50' : 'border-emerald-200 text-emerald-700 hover:bg-emerald-50'"><PowerOff v-if="v.status === 'active'" class="h-4 w-4" /><Power v-else class="h-4 w-4" /></button></div></div><p v-if="!vehicles.data.length" class="py-4 text-center text-sm text-slate-500">No hay vehiculos con esos filtros.</p></div>
