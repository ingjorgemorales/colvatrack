...
const filteredRows = computed(() => (props.activity?.rows ?? [])
  .filter(row => status.value === 'todos' || row.status === status.value)
  .filter(row => `${row.plate} ${row.driver}`.toLowerCase().includes(search.value.toLowerCase()))
);

const movingRows = computed(() => filteredRows.value.filter(row => row.status === 'moving').length);
const stoppedRows = computed(() => filteredRows.value.filter(row => row.status === 'stopped').length);
const totalPoints = computed(() => filteredRows.value.reduce((sum, row) => sum + Number(row.points_count || 0), 0));
const totalDistance = computed(() => filteredRows.value.reduce((sum, row) => sum + Number(row.distance_km || 0), 0).toFixed(2));
  ...
  <AppLayout title="Actividad GPS de vehiculos">
    ...
    <section class="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
      <div class="mb-5 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        ...
        <form class="grid gap-3 md:grid-cols-[1fr_1fr_1fr_1fr_auto]" @submit.prevent="applyDateFilters">
      ...
      <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
        <article class="rounded-md border border-slate-200 bg-slate-50 p-4">
          ...
          <p class="mt-2 text-2xl font-bold text-slate-950">{{ filteredRows.length }}</p>
        </article>
        <article class="rounded-md border border-slate-200 bg-slate-50 p-4">
    ...
    <section class="mt-6 rounded-md border border-slate-200 bg-white p-5 shadow-sm">
      <div class="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <span class="text-sm text-slate-500">Mostrando {{ filteredRows.length }} de {{ activity.rows.length }} vehiculos activos</span>
      </div>
      ...
      <div class="space-y-3 lg:hidden">
        <article v-for="row in filteredRows" :key="row.id" class="rounded-md border border-slate-100 bg-slate-50 p-4 text-sm">
        ...
        <p v-if="!filteredRows.length" class="py-6 text-center text-sm text-slate-500">No hay vehiculos con esos filtros.</p>
      </div>
    </section>
  </AppLayout>
</template>
