  ...
  <AppLayout title="Dashboard">
    ...
    <section class="mt-6 grid gap-6 xl:grid-cols-[1.2fr_.8fr]">
      <div class="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div class="space-y-2 sm:hidden"><div v-for="item in recentRequests" :key="item.id" class="rounded border border-slate-100 bg-slate-50 p-3 text-sm"><div class="mb-1 flex items-start justify-between gap-2"><div class="font-semibold">#{{ item.id }} - <span class="text-slate-950">{{ item.vehicle?.plate ?? '-' }}</span></div><span class="shrink-0 rounded bg-[#e6eef7] px-2 py-1 text-xs text-[#123f6e]">{{ item.status }}</span></div><div class="grid grid-cols-2 gap-x-3 gap-y-1 text-xs text-slate-600"><div><span class="font-medium text-slate-700">Tecnico:</span> {{ item.technician?.name ?? '-' }}</div><div><span class="font-medium text-slate-700">Conductor:</span> {{ item.driver?.name ?? '-' }}</div></div></div><p v-if="!recentRequests.length" class="py-4 text-center text-sm text-slate-500">Sin solicitudes registradas</p></div>
      </div>
      <div class="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
